...
import com.module.user.utils.HttpUtils;
...
public class UserController extends BaseController {
    ...
    public Object registerWeb(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
        Result<User> result = null;
        try {
            String strLoginName = HttpUtils.getParamAsString(request, "loginName");
            String strPassword = HttpUtils.getParamAsString(request, "password");
            String strVerifyCode = HttpUtils.getParamAsString(request, "vericode");
            if (StringUtils.isBlank(strLoginName)) {
                result = Result.buildFail("手机号码不能为空");
                return JSON.toJSON(result);
            }
            if (StringUtils.isBlank(strPassword)) {
                result = Result.buildFail("账号密码不能为空");
                return JSON.toJSON(result);
            }
            if (StringUtils.isBlank(strVerifyCode)) {
                result = Result.buildFail("手机验证码不能为空");
                return JSON.toJSON(result);
            }
